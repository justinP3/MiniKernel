...
void init_metrics(metrics_t* m) {
    ...
    m->procesos_atendidos_primera_vez = 0;
...
void registrar_tiempo_respuesta(metrics_t* m, int tiempo_llegada, int tiempo_inicio) {
    ...
    m->procesos_atendidos_primera_vez++;
    pthread_mutex_unlock(&m->metrics_mutex);
}
...
void* hilo_metricas(void* arg) {
    ...
    while (1) {
        ...
        if (m->procesos_atendidos_primera_vez > 0) {
            promedio_respuesta = (double)m->tiempo_respuesta_total / m->procesos_atendidos_primera_vez;
        }
